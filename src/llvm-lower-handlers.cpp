...
bool LowerExcHandlers::runOnFunction(Function &F) {
    auto except_enter_func = F.getParent()->getFunction("julia.except_enter");
    if (!except_enter_func)
        return false; // No EH frames in this module
    ensure_enter_function(*F.getParent());
    auto leave_func = F.getParent()->getFunction("jl_pop_handler");
    auto jlenter_func = F.getParent()->getNamedValue("jl_enter_handler");
    auto setjmp_func = F.getParent()->getNamedValue(jl_setjmp_name);
    auto lifetime_start = Intrinsic::getDeclaration(F.getParent(), Intrinsic::lifetime_start);
    auto lifetime_end = Intrinsic::getDeclaration(F.getParent(), Intrinsic::lifetime_end);
    ...
    for (int i = 0; i < MaxDepth; ++i) {
        buffs.push_back(new AllocaInst(Type::getInt8Ty(F.getContext()),
